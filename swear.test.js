...
describe('promises', () => {
  ...
  it('cannot call a function response', async () => {
    ...
    await expect(swear([true]).map(a => a)(a => a)).rejects.toMatchObject({ message });
    await expect(swear([3]).map(a => a)(a => a)).rejects.toMatchObject({ message });
    await expect(swear(['a']).map(a => a)(a => a)).rejects.toMatchObject({ message });
    await expect(swear([[]]).map(a => a)(a => a)).rejects.toMatchObject({ message });
    await expect(swear([{}]).map(a => a)(a => a)).rejects.toMatchObject({ message });
  });
  ...
  it('can catch a root error', async () => {
    expect(await swear(Promise.reject().catch(err => 'Hello'))).toEqual('Hello');
    expect(await swear(Promise.reject()).catch(err => 'Hello')).toEqual('Hello');
  });
  ...
  it('continues the chain after .catch()', async () => {
    ...
    expect(await swear(Promise.reject('Hello')).catch(err => 'a b')).toEqual('a b');
    expect(typeof swear(Promise.reject('Hello')).catch(err => 'a b').split).toBe('function');
    expect(await swear(Promise.reject()).abc('').catch(err => 'Hello')).toEqual('Hello');
  });

  it('ignores intermediate links in the chain until catch', async () => {
    expect(await swear(Promise.reject(new Error('rejected'))).split('').catch(err => err.message)).toEqual('rejected');
    expect(await swear(Promise.reject(new Error('rejected'))).abcde('').catch(err => err.message)).toEqual('rejected');
    expect(await swear(Promise.reject()).split('').catch(err => 'Hello')).toEqual('Hello');
    expect(await swear(Promise.reject()).abcde('').catch(err => 'Hello')).toEqual('Hello');
  });
  ...
  it('skips intermediate values', async () => {
    ...
    let called = 0;
    try {
      await swear(reject()).split('').map(a => called++);
    } catch (error) {
      err = error;
    }
...
describe('numbers', () => {
  ...
  it('can perform several operations', async () => {
    expect(await swear(3.1).toFixed(1).split('.').map(n => n * 2).join('.')).toBe('6.2');
  });
});
...
describe('arrays', () => {
  ...
  describe('.find()', () => {
    ...
    it('has all the right params for filter', async () => {
      expect(await swear([0, 1, 2]).find(async (a, i, all) => {
        expect(a).toEqual(i);
        expect(all).toEqual([0, 1, 2]);
        return a > 1;
      })).toEqual(2);
